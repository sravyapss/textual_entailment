...
from util import Attention1, Attention2, Softmax, DotProduct, GatedEncoder
import pdb
...
def train(epoch_no, data_train, compute_train_costerror):
    cost = 0.
    error = 0.
    for batch, i in enumerate(range(0, len(data_train), bsz)):
    	if(i+bsz>=len(data_train)):
	    break
...
def train_main():
    ...
    input_layer = lasagne.layers.InputLayer(shape=(bsz, None), input_var=sent)
    mask_layer = lasagne.layers.InputLayer(shape=(bsz, None), input_var=mask)
    ...
    sent1_layer = lasagne.layers.InputLayer(shape=(bsz, None), input_var=sent1)
    sent2_layer = lasagne.layers.InputLayer(shape=(bsz, None), input_var=sent2)

    mask1 = T.TensorType('int32', [False, False])('sentence_mask')
    mask2 = T.TensorType('int32', [False, False])('hypothesis_mask')
    mask1_layer = lasagne.layers.InputLayer(shape=(bsz, None), input_var=mask1)
    mask2_layer = lasagne.layers.InputLayer(shape=(bsz, None), input_var=mask2)
    ...
    params = lasagne.layers.get_all_params(MLP_output_layer)+ lasagne.layers.get_all_params(sentence_embed_layer)
    ...
    return data_train, data_val, data_test, compute_train_costerror, compute_costerror, params
def main():
